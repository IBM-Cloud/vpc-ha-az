...
resource "ibm_is_instance_group" "app_instance_group" {
  ...
  instance_template  = ibm_is_instance_template.app_template[0].id
  ...
  depends_on = [ibm_is_instance_template.app_template]
}
...
resource "ibm_is_instance_group_manager" "app_instance_group_manager" {
  ...
  instance_group       = ibm_is_instance_group.app_instance_group[0].id
...
resource "ibm_is_instance_group_manager_policy" "app_cpu_policy" {
  ...
  policy_type            = "target"
  instance_group         = ibm_is_instance_group.app_instance_group[0].id
  instance_group_manager = ibm_is_instance_group_manager.app_instance_group_manager[0].manager_id
  metric_type            = "cpu"
...
resource "time_sleep" "wait_600_seconds_app" {
  count           = var.enable_automation ? (var.enable_autoscaling ? 1 : 0) : 0
  create_duration = "600s"
  depends_on      = [ibm_is_instance_group_manager.app_instance_group_manager, ibm_is_instance_group_manager.web_instance_group_manager]
}
